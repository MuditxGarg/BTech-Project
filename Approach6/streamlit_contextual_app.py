...
# Define function to reset all fields
def reset_chat():
    st.session_state.history = []
...
with st.sidebar:
    st.button("New Chat", on_click=reset_chat)  # Reset all fields on new chat
    ...
    if st.button("Get Answer"):
        with st.spinner("Retrieving information..."):
            ...
            new_entry = {
                ...
                "response_translated": answer_translated.replace("", "") if language_code != "en" else "",
                "image": best_image
            }
            # Append the new entry regardless; if image is same as previous, avoid displaying
